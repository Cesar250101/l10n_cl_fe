...
class ResPartner(models.Model):
                    ...
                    try:
                        for r in self:
                            if (
                                ...
                                and not r.parent_id
                                and self.check_vat_cl(r.document_number.replace(".", "").replace("-", "")) if r.parent_id.document_type_id.name=='RUT' else r.document_number
                            ):
    ...
    def fill_partner(self):
        if self.sync:
            return
        if self.document_type_id.name=='RUT':
