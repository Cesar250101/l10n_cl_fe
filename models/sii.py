...
class sii_document_letter(models.Model):
    ...
    sii_document_class_ids = fields.One2many("sii.document_class", "document_letter_id", "SII Document Classes")
    issuer_ids = fields.Many2many(
        "sii.responsability", "sii_doc_letter_issuer_rel", "letter_id", "responsability_id", "Issuers",
    )
    receptor_ids = fields.Many2many(
        "sii.responsability", "sii_doc_letter_receptor_rel", "letter_id", "responsability_id", "Receptors",
    )
    active = fields.Boolean("Active", default=True)
    vat_discriminated = fields.Boolean(
        "Vat Discriminated on Invoices?", help="If True, the vat will be discriminated on invoice report."
    )
...
class sii_responsability(models.Model):
    ...
    issued_letter_ids = fields.Many2many(
        "sii.document_letter", "sii_doc_letter_issuer_rel", "responsability_id", "letter_id", "Issued Document Letters"
    )
        ...
        for r in self:
            if r.product_types:
                ...
                if not types.issubset(["adjust", "consu", "service"]):
                    raise Warning(
                        _(
                            "You provided an invalid list of product types.\
