...
class AccountInvoice(models.Model):
    ...
    # intercambio_realizado = fields.Boolean(string='Entercambio Realizado?')
    
    def send_exchange_2(self):
        fecha_desde= datetime.now()-timedelta(days=90)
        domain=[('sii_result','=','Proceso'),
                ('date_invoice','>=',fecha_desde)
                ]
        dtes=self.search(domain,limit=50)
        if dtes:
            for d in dtes:
                commercial_partner_id = d.commercial_partner_id or d.partner_id.commercial_partner_id
                att = d._create_attachment()
                if not commercial_partner_id.es_mipyme:
                    body = "XML de Intercambio DTE: %s" % (d.number)
                    subject = "XML de Intercambio DTE: %s" % (d.number)
                    dte_email_id = d.company_id.dte_email_id or d.env.user.company_id.dte_email_id
                    ...
                    for dte_email in dte_receptors:
                        ...
                        if dte_email.email in ["facturacionmipyme2@sii.cl", "facturacionmipyme@sii.cl"]:
                            resp = d.env["sii.respuesta.cliente"].sudo().search([("exchange_id", "=", att.id)])
                            resp.estado = "0"
                            continue
                        if not dte_email.email in email_to:
                            email_to += dte_email.email + ","
                    if email_to == "":
                        email_to=dte_email.dte_email
                    if email_to!=False or email_to!="":
                        try:
                            values = {
                                "res_id": d.id,
                            ...
                            send_mail = d.env["mail.mail"].sudo().create(values)
                            send_mail.send()
                            # d.intercambio_realizado=True
                        except:
                            pass
...
a VAT."""))
    ...
    def send_exchange(self):
        ...
        send_mail.send()
        # self.intercambio_realizado=True
