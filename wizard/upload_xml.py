...
class UploadXMLWizard(models.TransientModel):
    ...
    def _default_category(self):
        ...
        try:
            ...
            res = False
        if not res:
            res=self.env['product.category'].search([],limit=1).id
        return res
        ...
        if not product_id and self.type == "compras":
            ...
            if not product_id:
                if not self.pre_process:
                    ...
                    coma = ""
                    for c in line.findall("CdgItem"):
                        if c.find("TpoCodigo").text !=None:
                            code += coma + c.find("TpoCodigo").text + " " + c.find("VlrCodigo").text
                        else:
                            code += coma + " " + c.find("VlrCodigo").text
                        coma = ", "
                    return NmbItem + "" + code
    ...
    def _prepare_line(self, line, document_id, type, company_id, fpos_id, price_included=False, exenta=False):
        ...
        if DscItem is None:
            DscItem=line.find("NmbItem").text
        else:
            if DscItem.text is None:
                DscItem=line.find("NmbItem").text
        ...
        data.update(
            {
                ...
                "name": DscItem if DscItem is not None else line.find("NmbItem").text,
                "price_unit": price,
                "discount": discount,
        ...
        if self.pre_process and self.type == "compras":
            if DscItem:
                data.update(
                    {"new_product": product_id, "product_description": DscItem if DscItem is not None else "",}
                )
    ...
    def _create_inv(self, documento, company_id):
        ...
        ids_a_grabar=[]
        try:
            if data['purchase_to_done']:
                for d in data['purchase_to_done']:
                    ids_a_grabar.append(d.id)
                    inv.write(
                        {'purchase_id':d.id}
                    )
                    po=self.env['purchase.order'].search([('id','=',d.id)])
                    po.write(
                        {'invoice_ids':[(6,0,[inv.id])]}
                    )
                    for pol in po.order_line:
                        pol.write(
                            {'invoice_lines':[(6,0,[inv.id])]}
                        )
                    invoices = 0
                    for i in po.invoice_ids:
                        invoices +=1
                    po.invoice_count = invoices

                inv.write(
                    {'purchase_to_done':[(6,0,ids_a_grabar)]}
                )
        except:
            pass
